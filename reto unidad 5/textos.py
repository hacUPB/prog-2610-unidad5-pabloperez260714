...
from graficos import (grafico_palabras,grafico_histograma)
def leer_texto(ruta):
    try:
...
def resumen_texto(lineas):
    ...
    conectores = ["de", "la", "el","y", "en", "a","que", "los","las", "un","una"]
    ...
    print("\n resumen del texto ")
    print("Cantidad de líneas:",total_lineas)
...
def extraer_patrones(lineas):
    errores = 0
    for linea in lineas:
        if ("ERROR" in linea.upper() or "404" in linea):
...
def menu_textos(ruta):
    ...
    if len(lineas) == 0:
        return
    contador = resumen_texto(lineas)
    extraer_patrones(lineas)
    grafico_palabras(contador)
    grafico_histograma(lineas)